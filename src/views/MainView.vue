...
const showCardsPopup = ref(false);
...
const gameStore = ref(initialStorage ?? {
  collectedCards: [],
  mosaicParts: [],
});
const gameConfig = ref(config ?? {
  language: 'sl_m',
  firstTime: true,
});
...
function generateMosaic() {
  ...
  gameStore.value.mosaicParts = gameStore.value.mosaicParts.sort((a: any,b: any)=> a.z < b.z ? 1 : -1).reverse()
}
...
function collectedCardUpdated(cardKey: string) {
  ...
  gameStore.value = {
    ...
    mosaicParts: [...gameStore.value.mosaicParts, availableCards[cardKey].grants]
  };
...
<div class="bg-black text-white w-full h-full min-h-[90vh] flex flex-col">
  <template v-if="showLanguagePopup">
    <LanguageSelectPopup :config="gameConfig" @configUpdated="languageUpdated" @close="showLanguagePopup = false" />
  </template>
  ...
  <template v-if="showCardsPopup">
    <div class="fixed w-full h-full left-0 top-0 z-30 bg-black text-white">
      <div class="w-full mt-8 mb-2 border-b border-orange pb-2 px-4 flex flex-row items-center justify-between">
        <h1>{{ $t('popups.hintCardList.title') }}</h1>
        <button class="h-full bg-orange text-black px-4 py-2" @click="setCardsPopupState(false)">{{ $t('popups.close') }}</button>
      </div>
      <div class="flex flex-col gap-2">
        ...
        >
          <div class="flex flex-col flex-1">
            <div class="font-serif font-bold text-emphasis text-xl text-orange">
              {{ getOptionallyTranslatedText(availableCards[cardKey].title, locale) }}
            </div>
            <div class="text-white/50">
              {{ getOptionallyTranslatedText(availableCards[cardKey].name, locale) }}
            </div>
          </div>
          <a
          ...
          <button @click="setActiveCard(availableCards[cardKey])" class="text-orange border border-orange px-4 py-2 bg-gradient-to-b from-orange/25 to-orange/20">
            {{ $t('popups.open') }}
          </button>
        </div>
      </div>
  ...
  <div class="w-full flex flex-row justify-end">
    <div class="" @click="setCardsPopupState(true)">Moje kartice ({{ gameStore.collectedCards.length }})</div>
  </div>
  <div class="relative w-full h-full flex-1">
    <div v-for="mosaicPart of gameStore.mosaicParts" :key="mosaicPart.path"
